...
#include <bits/stdc++.h>
using namespace std;
bool f(int index1, int index2, string & pattern, string & text, vector<vector<int>> & dp){
   ...
   if(pattern[index1] == text[index2] || pattern[index1] == '?'){
      return dp[index1][index2] =  f(index1 - 1, index2 - 1, pattern, text,dp);
   }
   if(pattern[index1] == '*'){
      return dp[index1][index2] = f(index1 - 1, index2, pattern, text,dp) || f(index1, index2 - 1, pattern, text,dp);
   }
