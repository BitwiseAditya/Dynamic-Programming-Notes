...
For Example
If the given ‘HEIGHT’ array is [10,20,30,10], the answer 20 as the 
frog can jump from 1st stair to 2nd stair (|20-10| = 10 energy lost) and 
...
int frogJump(int n, vector<int> &heights)
{
    ...
    dp[0] = 0;
    for(int i=1; i<n; i++){
        int plusOne = dp[i-1] + abs(heights[i-1] - heights[i]);
        int plusTwo = INT_MAX;
        if(i>1)
        plusTwo = dp[i-2] + abs(heights[i-2] - heights[i]);
        dp[i] = min(plusTwo,plusOne);
    }
    return dp[n-1];
}

// can be further space optimisisd to O(1) space complexity.
