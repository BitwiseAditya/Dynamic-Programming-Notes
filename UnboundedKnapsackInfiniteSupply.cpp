...
You need to fill the knapsack with the items in such a way that you get the maximum profit. You are 
...
int f(int index, int w, vector<int> &profit, vector<int> &weight, vector<vector<int>> & dp){
    ...
    int notpick = 0 + f(index-1, w, profit, weight,dp);
    int pick = INT_MIN;
    if(w >= weight[index]) pick = profit[index] + f(index, w - weight[index], profit, weight, dp);
...
// Tabulation and space optimization later for revision practice.
