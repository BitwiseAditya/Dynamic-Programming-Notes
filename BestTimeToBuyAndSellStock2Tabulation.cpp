...
long getMaximumProfit(long *values, int n)
{
    ...
    for(int index=n-1; index>=0; index--){
        for(int investment=0; investment<=1; investment++){
            if(investment != 0){
                long sell = prices[index] + dp[index+1][0];
                long notsell = dp[index+1][1];
                dp[index][investment] = max(sell,notsell);
            }
            else{
                 long pick = -prices[index] + dp[index+1][1];
                 long notpick = dp[index+1][0];
                 dp[index][investment] = max(pick,notpick);
            }
        }
    }
