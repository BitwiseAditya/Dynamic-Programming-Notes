...
any one of these three activities. (Running, Fighting Practice or Learning New Moves). 
...
and activity. Your task is to calculate the maximum number of merit points that Ninja can earn.
...
int f(int i, int last, vector<vector<int>> & points, vector<vector<int>> & dp){
    ...
    for(int j = 0; j < 3; j++){
      if(j != last){
          int temp = points[i][j] + f(i-1,j,points,dp);
          maxi = max(maxi,temp);
      }
    }
