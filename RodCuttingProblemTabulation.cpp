/* Problem statement
Given a rod of length ‘N’ units. The rod can be
cut into different sizes and each size has a cost 
...
int cutRod(vector<int> &price, int n)
{
	...
	for(int len=0; len<=n; len++){
		if(len >= 1) dp[0][len] = (len/1)*price[0];
		else dp[0][len] = 0;
	}
	for(int ind=1; ind<n; ind++){
		for(int len=0; len<=n; len++){
	        ...
	        int pick = INT_MIN;
	        if(len >= ind + 1) pick = price[ind] + dp[ind][len - (ind + 1)];
