...
sizes and each size has a cost associated with it. Determine the maximum 
cost obtained by cutting the rod and selling its pieces.
...
#include <bits/stdc++.h>
using namespace std;
int f(int index, int remainingLength, vector<int> &price, vector<vector<int>> & dp)
{
	if(index == 0){
		if(remainingLength >= 1) return ((remainingLength/1)*price[0]);
		else return 0;
	}
	if(dp[index][remainingLength] != -1){
		return dp[index][remainingLength];
	}
	int notpick = 0 + f(index-1, remainingLength, price, dp);
	int pick = INT_MIN;
	if(remainingLength >= index + 1) pick = price[index] + f(index, remainingLength - (index + 1), price, dp);
